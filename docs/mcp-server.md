...
> ✅ **实现状态：已完成。** MCP 服务器已基于 TypeScript SDK v2 beta 落地于 `packages/backend/src/mcp/`，通过 `createMcpHandler` + `toNodeHandler` 接入 Fastify。
...
app.all(MCP_PATH, (request, reply) => nodeHandler(request.raw, reply.raw, request.body));
```
...
依据 `novel.ts` 中各方法的注释，**暴露给 LLM** 的方法（含 getter）为：

| `Novel` 成员                         | 对应 MCP 工具     | 暴露原因             |
| ------------------------------------ | ----------------- | -------------------- |
| `name` / `info`（getter）            | `get_info`        | 读取小说基本信息     |
...
读取当前小说的基本信息（对应 `Novel.name` / `Novel.info` getter；getter 不是 async 方法，因此在工具层补齐）。

| 入参   | 类型 | 说明                               |
| ------ | ---- | ---------------------------------- |
| （无） | —    | 目标小说由请求头 `X-Novel-Id` 决定 |

返回：`{ name: string, info: string }`。
